...
import { type Connector, type Connection, isArmResourceId } from '../../../utils/src';
  ...
  async getConnector(connectorId: string, getCached = false): Promise<Connector> {
    ...
    if (!isArmResourceId(connectorId)) {
      ...
      if (connectorIdKeyword === 'agent') {
        return {
          id: '/subscriptions/agentic/connector/agent',
          ...
          properties: {
            ...
            description: 'Agent Loop connector for agentic workflows',
            brandColor: '#0078D4',
            manifest: agentloop,
          },
        } as Connector;
